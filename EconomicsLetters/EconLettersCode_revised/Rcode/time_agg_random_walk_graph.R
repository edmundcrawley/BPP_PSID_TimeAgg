...
# points(time_agg_func, time, xlim= c(0,N_period),ylim=c(0,110000),col="blue",lty="dashed",lwd=4, verticals=TRUE ,col.points=FALSE)
...
underlying_func=stepfun(time[1:(N_subperiods-1)], underlying)
  ...
  pdf(paste(figures_dir, "TimeAgg_simple.pdf",sep=""))
  par(mar=c(5,7,2,2), mfcol=c(1,1),cex.axis=1.5,cex.lab=2)
  plot(underlying_func, time, xlim= c(0,N_period),ylim=c(0,110000),col="black",lty="solid",lwd=4, col.points=FALSE, verticals=TRUE,xlab="Time",main="",ylab="",yaxt = "n",xaxt = "n",las=1)
  axis(side = 1, at = c(0,time))
  axis(side = 2, at = c(0,25000,50000,75000,100000), las=1,label=c("$0","$25,000","$50,000","$75,000","$100,000"))
  points(obs_points+1, time_agg_func(obs_points),pch=4, cex=4, col="blue", lwd = 4)
  legend(0.5,30000,legend=c("Permanent Income Flow","Observed Annual Income"),lty=c("solid",NA), pch=c(NA, 4),lwd=c(4,4), pt.cex=4, col=c("black","blue"),bty = "n", cex=1.7)
  dev.off()

  pdf(paste(figures_dir, "TimeAgg_simple_1.pdf",sep=""))
  par(mar=c(5,7,2,2), mfcol=c(1,1),cex.axis=1.5,cex.lab=2)
  plot(underlying_func, time, xlim= c(0,N_period),ylim=c(0,110000),col="black",lty="solid",lwd=4, col.points=FALSE, verticals=TRUE,xlab="Time",main="",ylab="",yaxt = "n",xaxt = "n",las=1)
  axis(side = 1, at = c(0,time))
  axis(side = 2, at = c(0,25000,50000,75000,100000), las=1,label=c("$0","$25,000","$50,000","$75,000","$100,000"))
  legend(0.5,30000,legend=c("Permanent Income Flow",""),lty=c("solid",NA), pch=c(NA, NA),lwd=c(4,4), pt.cex=4, col=c("black","blue"),bty = "n", cex=1.7)
  dev.off()
