...
implied_cov_BPP <-function(params, ma, taste, varying_ins, T, perm_shk_params, tran_shk_params, perm_ins_params,tran_ins_params, meas_error_params) {
    if (ma==1) {
        teta <- params[1] 
    } else {
        teta <- 0.0
    ...
    for (j in 6:(T-3)){
        difcd[j,j]<-ins_perm[1+varying_ins]^2*var_perm[j-2]+ins_tran[1+varying_ins]^2*var_tran[j]+varcsi
    }

    difcd[T-2,T-2]<-ins_perm[1+varying_ins]^2*var_perm[T-4]+ins_tran[1+varying_ins]^2*var_tran[T-2]+varcsi
    difcd[T-1,T-1]<-ins_perm[1+varying_ins]^2*var_perm[T-4]+ins_tran[1+varying_ins]^2*var_tran[T-2]+varcsi
    difcd[T,T]    <-ins_perm[1+varying_ins]^2*var_perm[T-4]+ins_tran[1+varying_ins]^2*var_tran[T-2]+varcsi

    missing_v<-(var_c_error[1]+var_c_error[1]+var_c_error[2]+var_c_error[3]+var_c_error[4]+var_c_error[5]+var_c_error[6]+var_c_error[7]+var_c_error[8]+var_c_error[9])/9.0
    difcme[1,1]<-2*var_c_error[1]
    for (j in 2:7){
        difcme[j,j]<-var_c_error[j]+var_c_error[j-1]
    }

    difcme[8,8]<-var_c_error[7]+missing_v
    for (j in 9:11){
        difcme[j,j]<-2*missing_v
    }

    difcme[12,12]<-var_c_error[8]+missing_v
    difcme[13,13]<-var_c_error[9]+var_c_error[8]
    difcme[14,14]<-2*var_c_error[9]
    ...
    for (j in 7:(T-3)){
        difyc[j,j]<-ins_perm[1+varying_ins]*var_perm[j-2]+ins_tran[1+varying_ins]*var_tran[j]
    }
    difyc[T-2,T-2]<-ins_perm[1+varying_ins]*var_perm[T-4]+ins_tran[1+varying_ins]*var_tran[T-2]
    difyc[T-3,T-1]<-ins_perm[1+varying_ins]*var_perm[T-4]+ins_tran[1+varying_ins]*var_tran[T-2]
    difyc[T,T]    <-ins_perm[1+varying_ins]*var_perm[T-4]+ins_tran[1+varying_ins]*var_tran[T-2]

    for (j in 2:7){
        difyc[j-1,j]<- -(1-teta)*ins_tran[1]*var_tran[j-1]
    }
    for (j in 8:(T-1)){
        difyc[j-1,j]<- -(1-teta)*ins_tran[1+varying_ins]*var_tran[j-1]
    }

    difyc[T-1,T]<- -(1-teta)*ins_tran[1+varying_ins]*var_tran[T-2]

    for (j in 3:8){
        difyc[j-2,j]<- -teta*ins_tran[1]*var_tran[j-2]
    }
    for (j in 9:(T-1)){
        difyc[j-2,j]<- -teta*ins_tran[1+varying_ins]*var_tran[j-2]
    }
    #/* Final matrix */
...
BPP_parameter_estimation <- function(c_vector, omega, T, ma=1, taste=1, varying_ins=0){
    ...
    init_params[(1+ma+taste):(ma+taste+perm_shk_params)] <- matrix(0.03,nrow=perm_shk_params,ncol=1) 
    init_params[(1+ma+taste+perm_shk_params):(ma+taste+perm_shk_params+tran_shk_params)] <- matrix(0.03,nrow=tran_shk_params,ncol=1)
    ...
    objectiveFun <-function(params, ma, taste, varying_ins, T, empirical_cov, weight_matrix){
        model_cov <- implied_cov_BPP(params, ma, taste, varying_ins,T, perm_shk_params, tran_shk_params, perm_ins_params,tran_ins_params, meas_error_params)
    ...
    solved_objective <- nlm(objectiveFun, init_params, ma, taste, varying_ins, T, c_vector, weight_matrix, iterlim = 1000)
    solved_params <- solved_objective$estimate
    jacob <- jacobian(implied_cov_BPP, solved_params,  ma=ma, taste=taste, varying_ins=varying_ins,T=T, perm_shk_params=perm_shk_params, tran_shk_params=tran_shk_params, perm_ins_params= perm_ins_params,tran_ins_params=tran_ins_params, meas_error_params=meas_error_params)
