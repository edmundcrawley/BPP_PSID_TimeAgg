...
empirical_input_file = "C:/Users/edmun/OneDrive/Documents/Research/BPP_PSID_TimeAgg/Code/InputFiles/CohA.csv"
...
all_data1<- as.matrix(all_data ) 
    ...
    if (only_income) {
	output <- list("c_vector" = c_vector_income_only, "omega" = omega_income_only)
    }  else {
	output <- list("c_vector" = c_vector_both, "omega" = omega_both)
    }
    return (output) }
